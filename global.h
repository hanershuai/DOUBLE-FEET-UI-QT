...
typedef struct{
    ...
    double t;
    int dz;
}PID;//机器人步态的PID参数//定义在后面会报错，因为pid_debug会使用PID结构体
...
typedef struct{
    ...
    double  x;
    double y;
}Ball_info;//记录球在视野中的状态

typedef struct{
    int state;
    double left_goal;
    double right_goal;
}Goal_info;//记录视野中球门柱状态   (state)0:没有找到,1:找到一根门柱,3:找到两根门柱
