...
int Image_Init(void)
{
    if(Read_ColorTable() == 0)
            {
                cout<<"Can't read ColorTable"<<endl;
                return 0;
            }
    //选择摄像头
    My_Capture = cvCreateCameraCapture(-1);
    Image = cvQueryFrame(My_Capture);
    if(Image==NULL)
        cout<<"I can't get the image"<<endl;
    CvSize sz;
    //这里对图像改变大小
    ...
    cvNamedWindow("goal_image_gray",1);
...
void Image_Run()
{
    ...
    Image = cvQueryFrame(My_Capture);//读取视频帧
...
int Read_ColorTable()
{
    ...
    FILE  *fp_HSV;
          ...
          if((fp_HSV = fopen("/home/king/QT程序/ROBOT_QT/ColourTable_HSV.txt", "r+")) == NULL){
              cout<<"can't open Color_Table_HSV\n"<<endl;
              return 0;
          }
                     for( x=0;x<64;x++)
                                {
...
//		        					cout<<"重置指针位置"<<fp_position<<endl;
                ...
                if(flag==0)
                {
                 ...
                 if((fp_RGB = fopen("/home/king/QT程序/ROBOT_QT/ColourTable_RGB.txt", "r+")) == NULL){
...
//	cout<<"bein"<<endl;
          ...
          if((fp_HSV = fopen("/home/king/QT程序/ROBOT_QT/ColourTable_HSV_near.txt", "r+")) == NULL){
              cout<<"can't open the file\n"<<endl;
              return 0;
          }
    //	  cout<<"have open"<<endl;
